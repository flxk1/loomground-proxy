...
def test_movements_and_substitutions_are_never_derived():
    ...
    from loomground_proxy import proxy as mod
...
def test_every_substitution_carries_the_shape_a_brief_reads():
    # As in the mandate package: the rendering surface is a separate repository
    # and is not imported. `ref` is a property here rather than a field, and that
    # is exactly the kind of freedom testing the contract preserves.
    subs = check_proxies([SERVED], {"response_time": M.IMPROVED,
                                    "client_was_served": M.WORSENED})
    assert subs, "expected something to report"
    for s in subs:
        assert s.kind in KINDS
        assert s.ref and s.why
